...
const analyzeEmotionalLanguage = (text) => {
  ...
  return {
    ...
    score: Math.min(
      count * 5 + excessiveCaps * 10 + excessiveExclamation * 8,
      25
    ),
  };
};
...
const analyzeWritingQuality = (text) => {
  ...
  const grammarIssues = (text.match(/\b\w+\b\s+\b\w+\b/g) || []).filter(
    (phrase) => {
  ...
  return {
    ...
    score: Math.max(
      0,
      20 - shortSentences * 2 - longSentences * 1 - grammarIssues * 3
    ),
  };
};
...
const analyzeClickbaitScore = (text) => {
  const clickbaitIndicators = {
    ...
    urgencyWords: ["now", "today", "immediately", "urgent", "asap"].filter(
      (word) => text.toLowerCase().includes(word)
    ).length,
  };

  const score =
    Object.values(clickbaitIndicators).reduce((sum, val) => sum + val, 0) * 2;
...
const generateUncertainReasons = (analysis) => {
  ...
  if (
    analysis.sensationalWords.count > 0 &&
    analysis.sourceMentions.sourceMentions > 0
  ) {
    reasons.push("Mixed signals: sensational language with sources");
  }
  if (
    analysis.writingQuality.score >= 10 &&
    analysis.writingQuality.score <= 15
  ) {
    reasons.push("Average writing quality, needs more analysis");
  }
  if (
    analysis.emotionalLanguage.score > 0 &&
    analysis.emotionalLanguage.score < 15
  ) {
